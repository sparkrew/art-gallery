(() => {
  ...
  async function init() {
    ...
    O_policeexquise = await loadFont(
      "../art/js/2025/exquis/asset/FreeMono.otf"
    );
    console.log("police", O_policeexquise);
    O_configurationexquise = await loadJSON(
      "../art/js/2025/exquis/exquisite.json"
    );
    console.log("config", O_configurationexquise);
    // Get the number of artworks
    O_nbartworks = Object.keys(O_configurationexquise).length;

    console.log("config", O_configurationexquise);
    // Create the canvas
    O_widthexquis = O_widthCanva;
    O_heightexquis = O_heightCanva; //O_widthexquis / 1.82);
  ...
  async function initallworks() {
    ...
    for (let i = 0; i < O_nbartworks; i++) {
      ...
      console.log(O_configurationexquise);
  ...
  function draw() {
    ...
    if (index > 0) {
      console.log(
        frameRate() + " " + O_configurationexquise[index - 1].art_code
      );
    }
  }
